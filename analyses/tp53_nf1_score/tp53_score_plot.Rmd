...
tp53_survival <- hist %>%
  ...
  mutate(!grepl("^C3", Kids_First_Biospecimen_ID)) %>%
...
GBM_survival <- hist %>% 
  ...
  select(HARMONY_Gender, HARMONY_age, tp53_score, Kids_First_Biospecimen_ID)
...
```{r}
p1 <- ggplot(tp53_survival, aes(x = reported_gender, y = tp53_score)) + 
      geom_boxplot() +
      geom_jitter(width = 0.3, alpha = 0.5) + 
      stat_compare_means(comparisons = list(c("Female", "Male"))) + 
      ggtitle(label = "HOPE cohort")

p2 <- ggplot(GBM_survival, aes(x = HARMONY_Gender, y = tp53_score)) + 
      geom_boxplot() +
      geom_jitter(width = 0.3, alpha = 0.5) + 
      stat_compare_means(comparisons = list(c("Female", "Male"))) + 
      ggtitle(label = "CPTAC cohort")
...
```{r}
tp53_survival_filtered <- tp53_survival %>% 
  filter(!is.na(tp53_score))

p1 <- ggplot(tp53_survival_filtered, aes(x = broad_mol, y = tp53_score)) + 
  geom_boxplot() +
  geom_jitter(width = 0.3, alpha = 0.5) 

p2 <- ggplot(tp53_survival_filtered, aes(x = factor(mol_subtype_without_tp53, 
                                           levels = c("HGG, H3 wildtype", "HGG, IDH",
                                                      "DMG, H3 K28", "DHG, H3 G35",  
                                                      "IHG", "PXA")), y = tp53_score)) + 
  geom_boxplot() +
  xlab("Molecular_subtype") +
  geom_jitter(width = 0.3, alpha = 0.5) 
