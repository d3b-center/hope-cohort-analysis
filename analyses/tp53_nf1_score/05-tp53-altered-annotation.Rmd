...
# histology
clinical<-read.delim(file.path(data_dir,"Hope-GBM-histologies-base.tsv"))
...
histology <- clinical %>%
  dplyr::select("Kids_First_Biospecimen_ID",
        ...
        "composition",
        "cohort") 
gbm_muts <- read_tsv(file.path(input_dir, "harmonized_mutation_06082023.tsv"))
...
## generate a empty file for sv_overlap ##
sv_overlap <- data.frame(SV_chrom = NA, SV_start = NA, SV_end = NA, 
                         SV_length = NA, Kids.First.Biospecimen.ID.Tumor = NA, SV_type = NA, 
                         Gene_name = NA, ALT = NA, sample_id = NA)
...
consensus_tp53_snv_indel <- data.table::fread(
  file.path(data_dir,"Hope-snv-consensus-plus-hotspots.maf.tsv.gz"), select = 
  c("Chromosome",
         "Start_Position",
         "End_Position",
         "Strand",
...
tp53_alterations <- histology %>%
  filter(experimental_strategy %in% c("WGS", "WES")) %>%
  # filter for Tumors
...
```{r}
gbm_tp53 <- gbm_muts %>%
  filter(ApprovedGeneSymbol == "TP53") %>%
  pivot_longer(cols = -ApprovedGeneSymbol, names_to = "sample_id", values_to = "GBM_TP53_SNV") %>%
  select(-ApprovedGeneSymbol) %>%
  filter(grepl("^C3", sample_id))

tp53_alterations_score_wide <- tp53_alterations_score_wide %>%
  left_join(gbm_tp53, by = "sample_id") %>%
  mutate(SNV_indel_counts = case_when(GBM_TP53_SNV == 1 ~ 1,
                                      GBM_TP53_SNV == 0 ~ 0,
                                      TRUE ~ SNV_indel_counts)) %>%
  select(-GBM_TP53_SNV)
...
expr_combined <- readRDS(file.path(data_dir,"Hope-and-CPTAC-GBM-gene-expression-rsem-tpm-collapsed.rds"))
