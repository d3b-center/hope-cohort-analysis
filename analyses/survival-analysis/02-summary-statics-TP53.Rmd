...
tp53_score <- tp53_score %>% 
  mutate(SNV_alt = case_when(!is.na(HGVSp_Short) ~ "SNV_alt", TRUE ~ "NO"),
         CNV_alt = case_when(!is.na(CNV_loss_evidence) ~ "CNV_loss", TRUE ~ "NO"), 
         Fusion_alt = case_when(!is.na(Fusion_evidence) ~ "Fusion", TRUE ~ "NO"), 
         SV_alt = case_when(!is.na(SV_type) ~ "SV_alt", TRUE ~ "NO")) %>% 
  mutate(status = paste(SNV_alt, CNV_alt, Fusion_alt, SV_alt, sep = ","), 
         status = gsub(",NO", "", status), 
         status = gsub("NO,", "", status))
...
HGG_survival <- hist %>%
  ...
  mutate(binned_age = case_when(age_at_diagnosis_year >= 0 & 
                                ...
                                TRUE ~ "not reported")) %>%
  distinct() %>% 
  write_tsv(file.path(file.path(analyses_dir, "input", "HGG_survial_tp53.tsv")))
...
GBM_df <- hist %>%
    ...
    distinct() %>% 
    write_tsv(file.path(file.path(analyses_dir, "input", "GBM_survial_tp53.tsv")))
...
```{r}
ggsave(file.path(plots_dir, "boxplot_tp53_score_binned_age.png"))
...
```{r}
ggsave(file.path(plots_dir, "boxplot_tp53_score_status.png"))
...
```{r}
ggsave(file.path(plots_dir, "boxplot_tp53_score_gender.png"), height = 6, width = 12)
...
```{r}
ggsave(file.path(plots_dir, "boxplot_tp53_score_race.png"))
...
```{r}
ggsave(file.path(plots_dir, "boxplot_tp53_score_subtype.png"))
