...
bed_file <- file.path(root_dir, 
                      "scratch", 
                      "gencode.v39.primary_assembly.annotation.bed")
...
subset_bsid <- read_tsv(file.path(input_dir, "hgg_metadata.tsv")) %>%
...
> Any sample that harbors _H3F3A_ (_H3-3A_) K28M, _H3F3A_ (_H3-3A_) G35R/V, or _HIST1H3B_ (_H3C2_) K28M, if not classified as a high-grade glioma, should be, as this is a defining lesion. Eg: three former PNET tumors were re-classified as such.
...
These tumors do not have _H3-3A_ or _IDH1_ mutations, but harbor _TP53_ and _TERT_ mutations.
...
This will contain duplicated information -- e.g., _H3-3A_ will be present for tumors with _H3-3A_ K28M mutations -- but serve as a "sanity check."
...
idh_summary_df <- idh_snv_df %>%
  ...
  dplyr::bind_rows(data.frame(
    Tumor_Sample_Barcode = setdiff(subset_bsid,
                                   idh_snv_df$Kids_First_Biospecimen_ID)
  )) %>%
...
braf_summary_df <- braf_snv_df %>%
  ...
  dplyr::bind_rows(data.frame(
    Tumor_Sample_Barcode = setdiff(subset_bsid,
                                   braf_snv_df$Kids_First_Biospecimen_ID)
  )) %>%
