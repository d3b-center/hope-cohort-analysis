...
HGNC_symbol <- readr::read_tsv("analyses/proteomic-data/input/hgnc_complete_set.txt")
prot_df <- data.table::fread(file.path(output_dir, "Hope_proteome_imputed_data_liftover.tsv.gz")) 
Pho_df <- data.table::fread(file.path(output_dir, "Hope_phosphosite_imputed_data_ischemia_removed_liftover.tsv.gz"))
...
label_dup(Pho_df, file_dir = file.path(output_dir, "Hope_phosphosite_imputed_data_ischemia_removed_liftover.tsv.gz"))
