...
ggsave(filename = file.path(results_dir, "msi_vs_tmb.png"), height = 6, width = 8)
...
p <- ggplot(plot_data, aes(x = age_three_groups, y = msi_tumor_only, color = as.character(age_three_groups))) +
  stat_boxplot(geom ='errorbar', width = 0.2) +
  geom_boxplot(lwd = 0.5, fatten = 0.5, outlier.shape = 1, width = 0.4, outlier.size = 1) +
  geom_text_repel(aes(label = Type), na.rm = TRUE, hjust = 0, vjust = 0, size = 3, color = "black") +
  ggpubr::theme_pubr(base_size = 10) + ylab("") + 
...
p <- ggplot(plot_data, aes(x = age_two_groups, y = msi_tumor_only, color = as.character(age_two_groups))) +
  stat_boxplot(geom ='errorbar', width = 0.2) +
  geom_boxplot(lwd = 0.5, fatten = 0.5, outlier.shape = 1, width = 0.4, outlier.size = 1) +
  geom_text_repel(aes(label = Type), na.rm = TRUE, hjust = 0, vjust = 0, size = 3, color = "black") +
  ggpubr::theme_pubr(base_size = 10) + ylab("") + 
