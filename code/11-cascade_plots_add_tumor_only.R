...
output_dir <- file.path(root_dir, "results", "cascade_plots_add_tumor_only")
dir.create(output_dir, recursive = T, showWarnings = F)

# matrix
mat = read.table(file.path("results", "oncoprint_add_tumor_only.txt"),  header = TRUE, stringsAsFactors=FALSE, sep = "\t",check.names = FALSE)
mat[is.na(mat)] = ""
rownames(mat) = mat[, 1]
mat = mat[, -1]
mat = t(as.matrix(mat))
...
annot_info <- read.delim(file.path("results", "annotation_add_tumor_only.txt"), header = TRUE, check.names = TRUE)
